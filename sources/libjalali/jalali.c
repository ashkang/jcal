...
jalali_is_jleap(int year)
{
	...
	if ((pr >= cycle_patterns[i]) && (pr < cycle_patterns[i+1]))
	{
	    ...
	    if (!pr) /* pr is zero */
		return 0;
	    /* If year is a multiple of four then it's leap, ordinary otherwise. */
	    else
		return !(pr % J_LI);
	}
    }
...
jalali_get_date(int p, struct jtm* j)
{
    ...
    while (1) {
	d = (p >= 0 ) ? 1 : -1;
	f = jalali_is_jleap(((d > 0) ? y : y-1)) ? JALALI_LEAP_YEAR_LENGTH_IN_DAYS:
...
/*
 * Displays a jalali date struct fields. should be used for debugging purposes only.
 */
void
jalali_show_time(const struct jtm* j) {
    printf("%d/%02d/%02d (%02d:%02d:%02d) [%d], yday: %d, dst: %d, off: %ld, zone: %s\n",
	   j->tm_year, j->tm_mon + 1, j->tm_mday, j->tm_hour, j->tm_min, j->tm_sec,
	   j->tm_wday, j->tm_yday, j->tm_isdst, j->tm_gmtoff, j->tm_zone);
}
