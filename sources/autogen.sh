...
function clean() {
	...
	printk ${STAT}
	if [ ${STAT} -ne 0 ]; then
	    echo -e "${RED}error${RESET}: cannot perform make distclean. run make distclean manually and check for erros."
	fi
    fi
...
# $EXIT specifies whther is_present() should exit on the event of service not found.
function is_present() {
    local SERVICE=$1
    local NAME=$2
    local OUTPUT=$3
	...
	echo -e "${RED}error${RESET}: ${YELLOW}${NAME}${RESET} was not found on your system. autogen.sh cannot continue."
	exit 1
    fi
...
# $NAME is the service name.
# $EXIT specifies whether perform() should exit on the event of encoutering any errors or not.
...
function perform() {
    ...
    if [ ${STAT} -ne 0 ]; then
	echo -e "${RED}error${RESET}: cannot run ${YELLOW}${NAME}${RESET}. please run ${NAME} manually and check for errors."
    fi
...
if [ -z ${GETOPT} ]; then
    echo -e "warning: getopt(1) was not found on your system. command line arguments will be ignored."
else
